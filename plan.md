...
| M1 W6 bench gate status | **PARTIAL.** Incremental p95 < 250 ms ✓ (120 ms synthetic, 68 ms ripgrep). Warm index ≥ cmm ✓ (Grafy 1.5–2.3× faster). Peak RSS ≤ cmm ✓ (Grafy 0.4× cmm). **Cold index ≥ 2× faster than cmm ❌ FAIL** — Grafy 1.7–22.8× slower (ripgrep 9.1 s vs cmm 0.4 s). Root cause: W3 caller fan-out (~277k edges vs cmm's 13.5k) + pass-3 reparse. Both fixes spec'd in plan §8 and required before "drop-in faster" ship. Full report: `benches/m1-report.md`. Commits `80e7cab` + `5523639`. |
| Cold-index gate failure | **Blocks M1 close.** Two fixes required (both deferred from earlier weeks per plan §8): enclosing-function walk in `pass3.rs::process_calls_for_file` (replaces whole-file attribution); cached `tree_sitter::Tree` from pass 1 reused by pass 3 (eliminates reparse). Plan: ship as "W6.5" before tagging M1. |
